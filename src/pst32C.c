...
void norm(VECTOR v, type* result);
void div_scalare(VECTOR v, type val);
type prodotto_scalare(VECTOR v1, VECTOR v2);
void prodotto_vettore_scalare(VECTOR v, type scalar);
void prodotto_vettore_matrice(VECTOR v, MATRIX m, VECTOR result);
void sum(VECTOR v1, VECTOR v2, VECTOR result);
type euclidean(VECTOR v1, VECTOR v2);
...
void rotation(VECTOR axis, type theta, MATRIX rotation_matrix){
	...
	type a, b, c, d;
	type val;
	val = prodotto_scalare(axis, axis);
...
type hydrophobic_energy(char *s, MATRIX coords){
	...
	for(i=0; i<n ; i++){
		...
		for(j=i+1; j<n ; j++){
			...
			type dist;
			dist =euclidean(coords_c_alpha_i, coords_c_alpha_j);
			int pos_i = s[i]-65;
            int pos_j = s[j]-65;
...
void sum(VECTOR coords_c_alpha, VECTOR coords_n, VECTOR v){
	for (int i = 0; i < 3; i++) {
        v[i] = coords_c_alpha[i] + coords_n[i];
    }
}

void sub(VECTOR coords_c_alpha, VECTOR coords_n, VECTOR v){
	for (int i = 0; i < 3; i++) {
        v[i] = coords_c_alpha[i] - coords_n[i];
    }
}
